...
pf = input("Insert pcap file name written with the dir pcap/ ").strip()
    ...
    if first_time == 0:
        ...
        for (ipsrc, ipdst), flow in flows.items():
            if flow['packet_lengths']:  
                ...
                bitsec = (bytes_sum * 8) / int(interval)  # in bit/s
                current_throughput = round(bitsec)

                # Scrive riga per flusso
                writer2.writerow([str(time_s), ipsrc, ipdst, current_throughput])

                # Salva nel dizionario flusso
                flow['throughput'] = current_throughput

        # reset contatori pacchetti dei flussi per il prossimo intervallo
...
with open(detailed_csv_path, 'w', newline='') as csvfile, \
     open(flow_file, 'a') as flow_file, \
     open(f'output_pcap/traffic_components_{t}.csv', 'w', newline='') as csv_ipfile:

    writer = csv.writer(csvfile)
    writer.writerow(['ipsrc', 'ipdst', 'timestamp', 'src_port', 'dst_port', 'protocol', 'flags', 'packet_length', 'throughput'])

    csv_ip_writer = csv.writer(csv_ipfile)
    csv_ip_writer.writerow(['ipsrc', 'ipdst'])

    z = 0
    for (ipsrc, ipdst), flow in flows.items():
        csv_ip_writer.writerow([ipsrc, ipdst])

        if flow["protocols"][0] == "GTP" and flow["dst_ports"][0] == 2152:
            flow_file.write(f"Flow n. {z} from {ipsrc} to {ipdst} -- GTP --\n")
        elif flow["protocols"][0] == "GTP<DNS>" and flow["dst_ports"][0] == 53:
            flow_file.write(f"Flow n. {z} from {ipsrc} to {ipdst} -- GTP <DNS> -- GPRS Tunneling\n")
        else:
            flow_file.write(f"Flow n. {z} from {ipsrc} to {ipdst}\n")
        z += 1
        
        #throughput per singolo flusso
        for (ipsrc, ipdst), flow in flows.items():
            if "throughput" in flow:
                writer.writerow([
                    ipsrc,
                    ipdst,
                    time_sec,   # timestamp dell’intervallo
                    None,       # src_port non serve in aggregato
                    None,       # dst_port idem
                    flow["protocols"][0] if flow["protocols"] else None,
                    "",         # flags non servono in aggregato
                    sum(flow["packet_lengths"]),  # totale byte pacchetti
                    flow["throughput"]            # throughput medio dell’intervallo
                ])
