...
class MininetRest():
    ...
    def __init__(self, net):
        ...
        @self.app.get("/")
        async def read_root():
            return {"MARE Network Digital Twin": "Ready on HTTPS"}

        @self.app.post("/SF/NDT_traffic_monitoring")
        async def read_root():
            return {"MARE Network Digital Twin": "NDT Traffic Monitoring SF active"}

        @self.app.post("/SF/NDT_anomaly_detection")
        async def read_root():
            return {"MARE Network Digital Twin": "NDT Anomaly Detection SF active"}
        ...
        @self.app.post("/container/{container_name}/cmd")
        async def run_cmd(container_name: str, cmd: MNCmd):
            node = self.net[container_name]
            args = cmd.command
            rest = args.split(' ')
            # Substitute IP addresses for node names in command
            # If updateIP() returns None, then use node name
            rest = [self.net[arg].defaultIntf().updateIP() or arg
                ...
                for arg in rest]
            rest = ' '.join(rest)
            sys_command = "./cmd_container.sh " + str(container_name) + " \"" + str(rest) + " &\""
            # print(sys_command)
	...
	# uvicorn.run(self.app, host=mininet_config["ipaddr"], port=int(mininet_config["port"])) 
        ...
        uvicorn.run(
            self.app,
            #"main:server.app",
            host=mininet_config["ipaddr"],
            port=8443,
            ssl_certfile="./cert.pem",
            ssl_keyfile="./key.pem",
        # if you prefer a context object, uvicorn supports ssl_context in some wrappers
        )
