...
echo "[HORSE SAN] Monitoring file: '$FILE_TO_MONITOR' for updates every ${SLEEP_INTERVAL} seconds."
...
echo "[HORSE SAN] Initial timestamp: $(date -d @"$LAST_MOD_TIMESTAMP")"
...
while true; do
  ...
  if [ "$CURRENT_MOD_TIMESTAMP" -ne "$LAST_MOD_TIMESTAMP" ]; then
    ...
    echo "[HORSE SAN] Input detected from EM"
    ./select_attack.sh
