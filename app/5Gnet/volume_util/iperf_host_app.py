...
def start_udp_iperf(m:msg_tuple):
    ...
    with p.stdout as pipe:
        ...
        # This is to avoid zoombie process; can zoombie processes can be an issue?
        p.terminate()
        time.sleep(0.5)
        p.poll()
...
#           Note: using iperf3 RTT is not availavle with stdout for on-line reporting
    ...
    # p = subprocess.Popen( ['iperf','-s', '-B', m.srv_ip, '-p', m.port, '-i', '1', '-e', '-x', 'CMSV', '-t', str(float(m.t_dur)) ], 
    #                         stdout=subprocess.PIPE,
    #                         universal_newlines=True )
    # first_sample_rx = False
    # with p.stdout as pipe:
    ...
    #         if first_sample_rx and float(lsplit[1])==0.0:
    #             continue
    #         rep = tcp_report_tuple_server(time.time(),float(lsplit[4]))
    #         rep_dict = dict(rep._asdict())
    #         r.rpush(f'iperf:tcp_srv:{m.ses_id}:ue{m.ue_id}', json.dumps(rep_dict))
    #         first_sample_rx = True
    #     # This is to avoid zoombie process; can zoombie processes can be an issue?
    #     p.terminate()
    #     time.sleep(0.5)
